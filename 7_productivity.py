...
for f_Mc in dPar['a_Mc']:
    ...
    #============================================================================================================
    a_N_FS    = np.zeros( len( dClust.keys()))
    a_N_AS    = np.zeros( len( dClust.keys()))
    a_MS_mag  = np.zeros( len( dClust.keys()))
    iCl = 0
    for sCl in dClust.keys():
        aEqID = np.unique( dClust[sCl].flatten())
        print 'cl: ', iCl+1,'out of: ', len( aEqID), aEqID
    ...
    np.savetxt( file_out, np.array([a_MS_mag, a_N_AS, a_N_FS]).T, fmt='%10.3f%10i%10i',
                header = 'MAG          N-AS          N-FS; note N_AS=0 highlights singles or FS only')
    iMc += 1
